...
var Pagination = (function(){
    ...
    var initialize = function(opts){
        p.opts = opts;
		if(!ich.pagination){
			ich.addTemplate('pagination', '<div class="pagination"><ul>{{#pages}}<li class="{{active}} {{className}}"><a href="" data-page="{{href}}">{{title}}</a></li>{{/pages}}</ul></div>');
		}
    ...
    var generatePages = function(){
        ...
        var first = onPage === 0;
        pages.push({href: 0, title: '«', active: first ? 'active' : '', className: 'first'});
        pages.push({href: Math.max(0, onPage - 1), title: '‹ Previous', active: first ? 'active' : '', className: 'prev'});

        for(var i = from; i < to; i++){
            pages.push({href: i, title: i + 1, active: i === onPage ? 'active' : '', className: 'page'});
        }

        var last = onPage === Math.min(totalPages - 1, onPage + 1);
        pages.push({href: Math.min(totalPages - 1, onPage + 1), title: 'Next ›', active: last ? 'active' : '', className: 'next'});
        pages.push({href: totalPages - 1, title: '»', active: last ? 'active' : '', className: 'last'});
